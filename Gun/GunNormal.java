...
public class GunNormal extends ApplicationAdapter implements Screen {
    //Экран игры режма Gun, сложности normal
    ...
    int bulletUpgraden;
    boolean canShot = true, bossExist = false, upBossExist = false;
    int scorePlayer = 0;
    int enemyDestroyed;
    ...
    public GunNormal(final FOG game){
        this.game = game;
    ...
    public void render(float delta) {
        ...
        for(Rectangle r: upBullets){
            ...
            if(r.getY() > screenHeight - row_height * 10) {
        ...
        if(HPPLayer <= 0){
            backgroundMusic.setLooping(false);
            backgroundMusic.stop();
            game.setScreen(new FailScreen(game, scorePlayer, 0, enemyDestroyed, 1));
            dispose();
        }

        if(HPBoss <= 0){
            game.setScreen(new WinScreen(game, scorePlayer, 1, enemyDestroyed, 0));
            dispose();
        }
    ...
    public void createPlayerBullet(){
        ...
        bulletP.setWidth(50 * ratW);
        bulletP.setHeight(50 * ratH);
