...
public class FindNormal extends ApplicationAdapter implements Screen {
    //Экран игры с режимом Find и сложностью Hard
    FOG game;
    ...
    public void render(float delta) {
        ...
        if (timer <= 0) {
            if (targetDone) {
                game.setScreen(new WinScreen(game, (int) looted, 1, (int) looted, 2));
                dispose();
            } else {
                game.setScreen(new FailScreen(game, (int) looted, 2, (int) looted, 1));
                dispose();
            }
        }
