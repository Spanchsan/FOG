...
public class ObjectNormal extends ApplicationAdapter implements Screen {
    //Экран игры режима Object сложности Normal
    FOG game;
    ...
    Array<com.mygdx.game.Object.Entity.Crystal> crystals;
    ...
    public void render(float delta) {
        ...
        stage.act();
        stage.draw();
        for(com.mygdx.game.Object.Entity.Crystal cr: crystals){
        ...
        if(timer < 0){
            if(goalPoint <= points){
                game.setScreen(new WinScreen(game, (int) points, 1, crystalDestroyed, 1));
                dispose();
            } else {
                game.setScreen(new FailScreen(game, (int) points, 1, crystalDestroyed, 1));
                dispose();
            }
        }
        ...
        if(System.currentTimeMillis() - lastCrystalSpawnTime > 1500){
            createCrystal();
        }
    }
    ...
    /*private void movementApply(){
        if(Gdx.input.isKeyPressed(Input.Keys.LEFT) || !rightHeaded ){
            xPlayer -= playerStep * Gdx.graphics.getDeltaTime();
        }

        if(Gdx.input.isKeyPressed(Input.Keys.RIGHT) || rightHeaded){
            xPlayer += playerStep * Gdx.graphics.getDeltaTime();
        }

        if(Gdx.input.isKeyPressed(Input.Keys.UP) || upHeaded){
            yPlayer += playerStep * Gdx.graphics.getDeltaTime();
        }

        if(Gdx.input.isKeyPressed(Input.Keys.DOWN) || !upHeaded){
            yPlayer -= playerStep * Gdx.graphics.getDeltaTime();
        }
    }*/
    //Функция для создания кристала
    private void createCrystal(){
        com.mygdx.game.Object.Entity.Crystal crystal = new Crystal(crystalHPMax);
