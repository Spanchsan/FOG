...
public class ObjectEasy extends ApplicationAdapter implements Screen {
    //Экран игры режима Object сложности Easy
    FOG game;
    ...
    public void render(float delta) {
        ...
        shapeRenderer.rect(playerR.getX(), playerR.getY(), playerR.getWidth(), playerR.getHeight());
        shapeRenderer.end();*/
        ...
        if(timer < 0){
            if(goalPoint <= points){
                game.setScreen(new WinScreen(game, (int) points, 0, crystalDestroyed, 1));
                dispose();
            } else {
                game.setScreen(new FailScreen(game, (int) points, 1, crystalDestroyed, 0));
                dispose();
            }
        }
        ...
        if(System.currentTimeMillis() - lastCrystalSpawnTime > 1000){
            createCrystal();
        }
    }
